...
public class IssuesApi {
    ...
    @PutMapping("/workspace/{workspaceId}/project/{projectId}/editIssue")
    ...
    @DeleteMapping("/workspace/{workspaceId}/project/{projectId}/deleteIssue/{issueId}")
    public ResponseEntity<String> deleteIssue(Authentication authentication, @PathVariable(value="projectId") int projectId, @PathVariable(value="issueId") int issueId) {
        ...
        Optional<String> accessTokenOptional = userService.getToken(userPrincipal.getId());
        if(accessTokenOptional.isPresent()) {
            String uri = String.format("%1s/projects/%2s/issues/%3s?access_token=%s",
                    gitLabBaseUrl, projectId, issueId ,accessTokenOptional.get());
